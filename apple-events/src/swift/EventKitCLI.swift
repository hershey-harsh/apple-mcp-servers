...
struct DeleteCalendarResult: Codable {
    let title: String
    let deleted = true
}
...
class RemindersManager {
    ...
    func createCalendar(title: String, color: String?) throws -> CalendarJSON {
        let calendar = EKCalendar(for: .event, eventStore: eventStore)
        calendar.title = title

        // Choose a writable source for the new calendar.
        // Prefer the source used for new events, then any CalDAV (e.g. iCloud), then local.
        if let defaultCal = eventStore.defaultCalendarForNewEvents,
           defaultCal.source.sourceType != .birthdays,
           defaultCal.source.sourceType != .subscribed {
            ...
            throw NSError(domain: "", code: 400, userInfo: [NSLocalizedDescriptionKey: "No writable calendar source available. Please ensure at least one calendar account exists."])
        }
        ...
        return calendar.toCalendarJSON()
    }

    func updateCalendar(currentName: String, newName: String?, color: String?) throws -> CalendarJSON {
        let calendar = try findCalendar(named: currentName)
        if let newName = newName, !newName.isEmpty {
    ...
    func deleteCalendar(title: String) throws {
        try eventStore.removeCalendar(try findCalendar(named: title), commit: true)
    }
...
func main() {
    ...
    let isCalendarAction = action == "read-events" || action == "read-calendars" || action == "create-event" || action == "update-event" || action == "delete-event" || action == "create-calendar" || action == "update-calendar" || action == "delete-calendar"
    ...
    func handleAction() {
        do {
            ...
            case "create-calendar":
                guard let title = parser.get("name") else { throw NSError(domain: "", code: 400, userInfo: [NSLocalizedDescriptionKey: "--name required."]) }
                print(String(data: try encoder.encode(StandardOutput(result: try manager.createCalendar(title: title, color: parser.get("color")))), encoding: .utf8)!)
            case "update-calendar":
                guard let name = parser.get("name") else { throw NSError(domain: "", code: 400, userInfo: [NSLocalizedDescriptionKey: "--name required."]) }
                print(String(data: try encoder.encode(StandardOutput(result: try manager.updateCalendar(currentName: name, newName: parser.get("newName"), color: parser.get("color")))), encoding: .utf8)!)
            case "delete-calendar":
                guard let title = parser.get("name") else { throw NSError(domain: "", code: 400, userInfo: [NSLocalizedDescriptionKey: "--name required."]) }
                try manager.deleteCalendar(title: title); print(String(data: try encoder.encode(StandardOutput(result: DeleteCalendarResult(title: title))), encoding: .utf8)!)
            case "create-event":
                guard let title = parser.get("title") else { throw NSError(domain: "", code: 400, userInfo: [NSLocalizedDescriptionKey: "--title required."]) }
