  ...
  return handleAsyncOperation(async () => {
    ...
    return formatListMarkdown(
      ...
      (calendar) => {
        ...
        return [
          `- ${calendar.title} (${calendar.account})${color} (ID: ${calendar.id})`,
        ];
      },
      'No calendars found.',
    );
...
export const handleCreateCalendar = async (
  args: CalendarsToolArgs,
): Promise<CallToolResult> => {
  return handleAsyncOperation(async () => {
    const validatedArgs = extractAndValidateArgs(args, CreateCalendarSchema);
    const calendar = await calendarRepository.createCalendar(
      validatedArgs.name,
      validatedArgs.color,
    );
...
export const handleUpdateCalendar = async (
  ...
  return handleAsyncOperation(async () => {
    const validatedArgs = extractAndValidateArgs(args, UpdateCalendarSchema);
    const calendar = await calendarRepository.updateCalendar(
      validatedArgs.name,
...
export const handleDeleteCalendar = async (
  args: CalendarsToolArgs,
): Promise<CallToolResult> => {
  return handleAsyncOperation(async () => {
    const validatedArgs = extractAndValidateArgs(args, DeleteCalendarSchema);
    await calendarRepository.deleteCalendar(validatedArgs.name);
    return formatDeleteMessage('calendar', validatedArgs.name, {
      useQuotes: true,
      useIdPrefix: false,
      usePeriod: true,
    });
